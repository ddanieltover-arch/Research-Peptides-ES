...
function renderLayout(title: string, preheader: string, bodyHtml: string) {
  ...
  const supportAddress = Deno.env.get('EMAIL_SUPPORT_ADDRESS') || 'info@researchpeptides.es';
...
async function sendEmail(to: string, subject: string, html: string, text: string, replyTo?: string) {
  ...
  await transporter.sendMail({
    from: Deno.env.get('SMTP_FROM') || 'Research Peptides ES <no-reply@researchpeptides.es>',
    to,
    subject,
    html,
    text,
...
async function processEvent(event: EmailEventRow) {
  const adminTo = Deno.env.get('EMAIL_ADMIN_TO') || 'info@researchpeptides.es';
